<x-filament::page>
    ...
    {{-- Tombol PDF dipindah ke atas --}}
    @if ($this->start_date && $this->end_date && count($rows) > 0)
                ...
                @forelse ($rows as $i => $r)
                    <tr class="odd:bg-white even:bg-gray-50/50">
                        ...
                        <td class="px-4 py-2.5 text-right">
                            Rp {{ number_format($r['pembulatan'] ?? 0, 0, ',', '.') }}
                        </td>

                        <td class="px-4 py-2.5 text-right">
                            Rp {{ number_format($r['kasbon'] ?? 0, 0, ',', '.') }}
                        </td>

                        <td class="px-4 py-2.5 text-right">
                            Rp {{ number_format($r['sisa_kasbon'] ?? 0, 0, ',', '.') }}
                        </td>
                ...
                <tfoot class="bg-gray-50 border-t">
                    <tr class="text-gray-900 font-semibold">
                        ...
                        <td class="px-4 py-3 text-right">
                            Rp {{ number_format($totals['pembulatan'], 0, ',', '.') }}
                        </td>

                        <td class="px-4 py-3 text-right">
                            Rp {{ number_format($totals['kasbon'], 0, ',', '.') }}
                        </td>

                        <td class="px-4 py-3 text-right">
                            Rp {{ number_format($totals['sisa_kasbon'], 0, ',', '.') }}
                        </td>

                        <td class="px-4 py-3 text-right">
                            Rp {{ number_format($totals['total_setelah_bon'], 0, ',', '.') }}
                        </td>
